...
function _getText($, el) {
    if (Array.isArray(el)) {
        ...
        logger.warn(`Received an unknown element while extracting content from HTML files. This should never happen. Please open an issue at https://github.com/cmfcmf/docusaurus-search-local/issues if you see this message (debug: got type ${el.type}).`);
        return "";
    }
}
function getText($, el) {
...
export function html2text(html, type, url = "?") {
    ...
    if (type === "docs" || type === "blog") {
        ...
        const tags = $("article footer ul[class^=tags_] li")
            ...
            .toArray();
        const preFirstHeadingText = getText($, $("article .markdown header, article .markdown h1")
            .first()
            .prevAll()
            .get()
            .slice()
        ...
        if (!$main.length) {
            logger.warn("Page has no <main>, therefore no content was indexed for this page.", { url });
        }
        return {
            pageTitle,
            sections: [
...
export function getDocusaurusTag(html) {
    const $ = cheerio.load(html);
