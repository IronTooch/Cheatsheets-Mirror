...
const languageSchema = Joi.string().valid("ar", "da", "de", "en", "es", "fi", "fr", "hi", "hu", "it", "ja", "nl", "no", "pt", "ro", "ru", "sv", "th", "tr", "vi", "zh");
const optionsSchema = Joi.object({
    ...
    language: Joi.alternatives(languageSchema, Joi.array().items(languageSchema)).default("en"),
...
export default function cmfcmfDocusaurusSearchLocal(context, options) {
    let { indexDocSidebarParentCategories, includeParentCategoriesInPageTitle, indexBlog, indexDocs, indexPages, language, style, maxSearchResults, lunr: { tokenizerSeparator: lunrTokenizerSeparator, k1, b, titleBoost, contentBoost, tagsBoost, parentCategoriesBoost, }, } = options;
  ...
  .map(token => token.str);\n`;
    ...
    return {
        ...
        getDefaultCodeTranslationMessages: async () => {
            const translationsDir = path.resolve(__dirname, "..", "..", "codeTranslations");
            ...
            for (const locale of localesToTry) {
                ...
                if (fs.existsSync(translationPath)) {
                    return JSON.parse(await fs.promises.readFile(translationPath, "utf8"));
                }
            }
            return {};
        },
        ...
        async postBuild({ routesPaths = [], outDir, baseUrl, siteConfig: { trailingSlash }, plugins, }) {
            ...
            const documents = (await Promise.all(data.map(async ({ file, url, type }) => {
                ...
                const { pageTitle, sections, docSidebarParentCategories } = html2text(html, type, url);
            ...
            logger.info(`${Object.keys(documentsByDocusaurusTag).length} indexes will be created.`);
            await Promise.all(Object.entries(documentsByDocusaurusTag).map(async ([docusaurusTag, documents]) => {
                logger.info(`Building index ${docusaurusTag} (${documents.length} documents)`);
                ...
                await writeFileAsync(path.join(outDir, `search-index-${docusaurusTag}.json`), JSON.stringify({
                    documents: documents.map(({ id, pageTitle, sectionTitle, sectionRoute, type, docSidebarParentCategories, }) => {
                        let fullTitle = pageTitle;
                        if (includeParentCategoriesInPageTitle &&
                            docSidebarParentCategories &&
